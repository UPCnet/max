...
class FunctionalTests(unittest.TestCase, MaxTestBase):
    ...
    def test_get_people_as_vip_without_subscriptions(self):
        """
    ...
    def test_get_people_as_non_vip_without_subscriptions(self):
        """
            Given i'm a not a vip user
            When I search users
            And we don't share any context subscription
            Then I cannot see any of them
        """
    ...
    def test_get_people_as_vip(self):
        """
            Given i'm a vip user
            When I search users
            Then I can see all people on the same contexts as I, including vips
        """
        from .mockers import subscribe_context, create_context
        usernamenotvip1 = 'user1'
        usernamenotvip2 = 'user2'
        ...
        self.create_context(create_context)
        self.admin_subscribe_user_to_context(usernamenotvip1, subscribe_context)
        self.admin_subscribe_user_to_context(usernamevip1, subscribe_context)
        self.admin_subscribe_user_to_context(usernamevip2, subscribe_context)
        ...
        res = self.testapp.get('/people', "", oauth2Header(usernamevip1), status=200)

        self.assertEqual(len(res.json), 3)
        self.assertEqual(res.json[0]['username'], usernamevip2)
        self.assertEqual(res.json[1]['username'], usernamevip1)
        self.assertEqual(res.json[2]['username'], usernamenotvip1)

    def test_start_conversation_with_people_being_a_vip_without_contexts(self):
        ...
        self.testapp.post('/conversations', json.dumps(message), oauth2Header(usernamevip1), status=401)
    ...
    def test_start_conversation_with_a_vip_not_being_a_vip_sharing_contexts(self):
        """
            Given i'm not a vip user
            When I try to start a conversation with a vip
            And i share some subscription with the vip
            Then I cannot start the conversation
        """
        from .mockers import message
        from .mockers import subscribe_context, create_context
        ...
        self.create_context(create_context)
        self.admin_subscribe_user_to_context(usernamenotvip, subscribe_context)
        self.admin_subscribe_user_to_context(usernamevip1, subscribe_context)
